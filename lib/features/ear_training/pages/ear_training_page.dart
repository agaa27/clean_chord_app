...
// Reuse model QuizLevel dari Kuis Chord, tapi datanya pakai earLevels
// (waktu per level udah disesuaikan khusus buat Ear Training).
import '../../kuis_chord/models/quiz_level_model.dart' show QuizLevel;
import '../models/ear_level_model.dart';
import 'ear_training_game_page.dart';
...
class EarTrainingPage extends StatelessWidget {
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(
      ...
      title: const Row(
        children: [
          Icon(Icons.headphones_rounded, color: Color(0xFF00E5FF), size: 20),
          SizedBox(width: 8),
          Text(
            'Ear Training',
            style: TextStyle(
  ...
  Widget build(BuildContext context) {
    final Map<String, List<QuizLevel>> grouped = {
      'Pemula':   earLevels.where((l) => l.difficulty == 'Pemula').toList(),
      'Menengah': earLevels.where((l) => l.difficulty == 'Menengah').toList(),
      'Mahir':    earLevels.where((l) => l.difficulty == 'Mahir').toList(),
    };
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.headphones_rounded, color: _cyan, size: 18),
                    ),
                    ...
                    const Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 2),
                          Text(
                            'Dengarkan suaranya, tebak nama chord-nya',
                  ...
                  children: [
                    ...
                    ...levels.map((level) {
                      final status = ProgressionService.instance.levelStatus(
                        ProgressionConfig.featureEar, level.id,
                      );
                      return _EarLevelCard(
                        level: level,
                        accentColor: color,
                        status: status,
                      );
...
class _EarLevelCard extends StatelessWidget {
          ...
          child: InkWell(
            ...
            onTap: isLocked
                ...
                : () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => EarTrainingGamePage(level: level),
                      ),
                    ),
            child: Container(
